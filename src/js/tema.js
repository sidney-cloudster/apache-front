...
var data = $("#chat-input").val();  
  ...
  clearInput();
  $(".ch-wrapper-p").stop().animate({ scrollTop: $(".ch-wrapper-p")[0].scrollHeight}, 1000);
});
...
$.validator.addMethod("birthday_date", function (value) {
  ...
  var pattern = /^(?=\d)(?:(?:31(?!.(?:0?[2469]|11))|(?:30|29)(?!.0?2)|29(?=.0?2.(?:(?:(?:1[6-9]|[2-9]\d)?(?:0[48]|[2468][048]|[13579][26])|(?:(?:16|[2468][048]|[3579][26])00)))(?:\x20|$))|(?:2[0-8]|1\d|0?[1-9]))([-.\/])(?:1[012]|0?[1-9])\1(?:1[6-9]|[2-9]\d)?\d\d(?:(?=\x20\d)\x20|$))?(((0?[1-9]|1[012])(:[0-5]\d){0,2}(\x20[AP]M))|([01]\d|2[0-3])(:[0-5]\d){1,2})?$/;	return pattern.test(value);
...
$('#comprar-form').append(item);
    ...
    $('#modal-fechar').click(function() {
      $(".modal-trigger").fadeOut(300, function() { $(this).remove(); });
      return false;
    });
...
// Setter
$( ".side-menu" ).menu( "option", "position", { my: "left top", at: "left+240 top-0 " } );
